...
int main (){
	...
	int r, t,g;
	cout << endl;
	cout << " masukan jumalh data : "; cin >> t;
	cout << " MAsukan pilihan ";cin >> r;
	do 
	switch(r){
	case 1 : 
	for (int i=0; i<= t; i ++){
		cout << " elemen ke"  << i +1 << endl;
		cout << "NPM ";cin >>  q.dat[i].npm;
		cout << "nama  "; cin  >> q.dat[i].nama;
		cout << "NIlai "; cin >> q.dat[i].nilai;
	}
	break;
	case 2 : 
	cout << " masukan jumlah yang ingion dihapus : ";cin >>g;
		for (int i=0;i <= g;i++){
			DEL(&q);
		}
	break;
